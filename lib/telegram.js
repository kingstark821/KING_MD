...
function createTelegramBot(onMessage) {
    ...
    bot.onText(/^\/pair(?:@\w+)?(?:\s+(.+))?$/i, async (msg, match) => {
        ...
        try {
            ...
            console.error(`❌ Pairing Telegram (${number}):`, e.message);
            await bot.sendMessage(chatId, '❌ Impossible de générer le Pair Code pour le moment. Vérifie le numéro et réessaie plus tard.');
        } finally {
